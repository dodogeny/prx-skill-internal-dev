...
- **Disk Monitor:** An optional in-process background thread (`workers/diskMonitor.js`) that tracks disk space consumed by `~/.prevoyant/` and overall machine disk capacity. Enabled via `PRX_DISK_MONITOR_ENABLED=Y` in Settings › Disk Monitor. Configurable check interval (`PRX_DISK_MONITOR_INTERVAL_MINS`, default 60), alert threshold (`PRX_DISK_CAPACITY_ALERT_PCT`, default 80%), and cleanup interval (`PRX_DISK_CLEANUP_INTERVAL_DAYS`, default 7 days). Sends an email alert when disk usage crosses the threshold (4-hour cooldown between repeated alerts). When the cleanup interval elapses, a pending-cleanup notification appears on the new **Disk Monitor page** (`/dashboard/disk`) — a dashboard **Approve Cleanup** button must be clicked before any files are deleted (no automatic deletion). An additional **Run Cleanup Now** button is always visible on the Disk Monitor page for on-demand house-cleaning at any time. Cleanup removes session directories older than 30 days and trims `disk-log.json` and `activity-log.json` to their most recent entries. The page shows a two-column table of what will be cleaned vs. what is permanently protected (knowledge base files, reports, `.env`, recent sessions). Knowledge base files are safeguarded at the route level — the resolved KB path is checked before any deletion. History of snapshots is persisted to `~/.prevoyant/server/disk-log.json` (up to 720 entries, ~30 days at hourly checks) and `~/.prevoyant/server/disk-status.json`. A usage-over-time chart (Chart.js) shows `.prevoyant` folder size and overall disk utilisation. A Disk icon in the dashboard header nav turns orange when cleanup is pending.
- **Claude Budget Tracker:** The dashboard shows real-time Claude API spend against the configured monthly budget (`PRX_MONTHLY_BUDGET`). Two cost sources are supported: (1) **Anthropic Cost Report API** — actual billed USD via `GET /v1/organizations/cost_report`, activated by setting `PRX_ANTHROPIC_ADMIN_KEY` to an organisation Admin API key (generate at platform.claude.com/settings/admin-keys); (2) **ccusage fallback** — calculated from local token counts when no admin key is set, labelled "ccusage calc'd" to signal it may diverge from actual billing. Token breakdown (input / cache-read / cache-write / output tokens) from ccusage is always shown in the card for transparency; when the Anthropic API is the cost source, the ccusage-calculated figure is shown alongside for comparison. Displayed in two places: a **Budget item in the info strip** (colour-coded remaining + source indicator) and a **Budget card** in the cards row with progress bar and per-token breakdown. Cache is 2 minutes; saving settings immediately busts the cache. Falls back gracefully if neither source is available.
